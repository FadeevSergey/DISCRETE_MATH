...
public class A {
    ...
    static void scanEdges() throws IOException {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader (new FileInputStream("fullham.in"), "UTF-8"))) {
    ...
    static void getSolution() {
        for(int i = 0, tempPosition; i < matrixEdges.size() * matrixEdges.size(); ++i) {
            if(!hasEdges(nodes.get(0), nodes.get(1))) {
                tempPosition = 2;
                while(!hasEdges(nodes.get(0), nodes.get(tempPosition)) || !hasEdges(nodes.get(1), nodes.get(tempPosition + 1))) {
                    ++tempPosition;
                }
    ...
    static void printSolution() throws IOException {
        try (PrintWriter out = new PrintWriter("fullham.out", "UTF-8")) {
            for(int i : nodes) {
                out.print((i + 1) + " ");
            }
        }
